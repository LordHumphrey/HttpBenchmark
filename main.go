...
func doDnsQueryWithRetry(httpBaseConfig *Common.HttpBaseConfig, host, subNetIp string, maxAttempts int) []*net.IP {
	var queryRes []*net.IP
	for i := 0; i < maxAttempts; i++ {
		queryRes = doDnsQuery(httpBaseConfig, host, subNetIp)
		if len(queryRes) != 0 {
			break
		}
	}
